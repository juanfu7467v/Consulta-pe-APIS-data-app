...
const creditosMiddleware = (costo) => {
  return async (req, res, next) => {
    ...
    if (req.user.tipoPlan === "creditos") {
      ...
      // ⭐ MEJORA: Obtener el saldo actual justo antes de la deducción atómica
...
/**
 * ⭐ MIDDLEWARE CORREGIDO: Gestiona el descuento de créditos aleatorio/cíclico (5, 6, 7, 8) para /api/ficha.
 * Asegura el descuento atómico y la actualización del contador.
 */
const creditosFichaMiddleware = async (req, res, next) => {
    ...
    if (req.user.tipoPlan === "creditos") {
        ...
        try {
            // Utilizamos una transacción para asegurar la consistencia al leer y escribir el contador y el usuario
            await db.runTransaction(async (t) => {
                const cycleDoc = await t.get(cycleRef);
                const userDoc = await t.get(userRef); // ⭐ LECTURA FRESCA DEL USUARIO

                let currentIndex = cycleDoc.exists ? cycleDoc.data().currentIndex || 0 : 0;
                let currentCredits = userDoc.exists ? userDoc.data().creditos ?? 0 : 0; // ⭐ SALDO FRESCO
                
                // 1. Determinar el costo actual y el próximo índice
                ...
                // 4. Deduce créditos y actualiza la última consulta y el dominio de uso
                t.update(userRef, {
                    // ⭐ DEDUCCIÓN SEGURA: Restamos el costo a la lectura fresca
            ...
            console.error("Error en transacción de Ficha:", e);
...
// ⭐ CLAVE: Endpoint /api/ficha USANDO EL MIDDLEWARE CORREGIDO
